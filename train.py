...
def random_flip_images(image_batch,label_batch,landmark_batch):
    ...
    if np.random.choice([0,1]) > 0:
        ...
        for i in fliplandmarkindexes:
            ...
            landmark_[[5, 6]] = landmark_[[6, 5]]#left mouth<->right mouth        
...
def train(netFactory, modelPrefix, endEpoch, dataPath, display=200, baseLr=0.01, gpus=""):
    ...
    landmark_target = tf.placeholder(tf.float32,shape=[config.BATCH_SIZE,18],name='landmark_target')
    #class,regression
...
def parse_args():
    ...
    parser.add_argument('--pretrained_model', dest='model', default='',type=str)
