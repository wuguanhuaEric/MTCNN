...
def P_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
	...
	first_net=conv4_1
        ...
        #batch*H*W*10
        landmark_pred = slim.conv2d(net, num_outputs=18, kernel_size=[1,1], stride=1, scope='conv4_3', activation_fn=None)
        if training:
            #batch*2
            ...
            return cls_pro_test, bbox_pred_test, landmark_pred_test,first_net, t_conv1
...
def R_Net(inputs,label=None, bbox_target=None, landmark_target=None, training=True):
    with slim.arg_scope([slim.conv2d],
        ...
        #batch*10
        landmark_pred = slim.fully_connected(fc1,num_outputs=18,scope="landmark_fc",activation_fn=None)
        #train
        if training:
            cls_loss = cls_ohem(cls_prob,label)
            bbox_loss = bbox_ohem(bbox_pred,bbox_target,label)
            ...
            return cls_prob,bbox_pred,landmark_pred,first_net, _out
